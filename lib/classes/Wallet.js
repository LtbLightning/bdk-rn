import { AddressInfo, Balance, FeeRate, FullScanRequest, SyncRequest, } from './Bindings';
import { createOutpoint, createTxOut, getNetwork } from '../lib/utils';
 ...
 */ export class Wallet extends NativeLoader {
     ...
     */
    async create(descriptor, changeDescriptor = null, network, persistenceBackendPath) {
        this.id = await this._bdk.walletNew(descriptor.id, changeDescriptor ? changeDescriptor.id : null, persistenceBackendPath, network);
        this.isInit = true;
        return this;
    }
    /**
    ...
    async listUnspent(walletId) {
        try {
            ...
            return outputs.map((item) => {
                return new LocalOutput(createOutpoint(item.outpoint), createTxOut(item.txout), item.keychain, item.isSpent);
            });
        }
        catch (error) {
    ...
    async listOutput() {
        ...
        return outputs.map((output) => {
            ...
            const keychain = output.keychain; // Convert to KeychainKind if necessary
     ...
     * @returns {Promise<string>} The wallet ID
     */
    async newNoPersist(descriptor, changeDescriptor, network) {
