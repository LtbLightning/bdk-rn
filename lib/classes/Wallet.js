...
import { AddressInfo, Balance, LocalUtxo, OutPoint } from './Bindings';
...
export class Wallet extends NativeLoader {
    ...
    async listUnspent() {
        ...
        let localUtxo = [];
        output.map((item) => {
            let localObj = new LocalUtxo(new OutPoint(item.outpoint.txid, item.outpoint.vout), createTxOut(item.txout), item.isSpent);
